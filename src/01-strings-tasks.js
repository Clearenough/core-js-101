...
function extractNameFromTemplate(value) {
  ...
  name = `${arr[1]} ${arr[2].slice(0, arr[2].length - 1)}`;
  return name;
}
...
function getRectangleString(width, height) {
  ...
  const res = top + vert.repeat(height - 2) + bot;
  return res;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => ROT13alphabet[alpabet.indexOf(letter)]);
}
